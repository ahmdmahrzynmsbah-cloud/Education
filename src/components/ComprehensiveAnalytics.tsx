...
// Generates simulated or filtered academic history for a student
const getStudentReportRecords = (studentId: string, rangeType: 'all' | 'month' | 'custom', monthStr?: string, startStr?: string, endStr?: string) => {
  const baseRecords = [
    { id: 'rec-1', type: 'اختبار دوري', name: 'اختبار النحو والبلاغة الدوري المقيد', details: 'الدرجة: 96 / 100', date: '2026-07-05', status: 'ممتاز' },
    { id: 'rec-2', type: 'امتحان شهري', name: 'الامتحان الشهري الموحد - الجبر وحساب المثلثات', details: 'الدرجة: 90 / 100', date: '2026-07-02', status: 'ممتاز' },
    { id: 'rec-3', type: 'اختبار علمي', name: 'اختبار الميكانيكا والكهرباء الحديثة الشامل', details: 'الدرجة: 88 / 100', date: '2026-06-25', status: 'جيد جداً' },
    { id: 'rec-4', type: 'واجب منزلي', name: 'تقييم القراءة والكتابة والبحث التعبيري الإبداعي', details: 'الدرجة: 92 / 100', date: '2026-06-18', status: 'ممتاز' },
    { id: 'rec-5', type: 'اختبار كيمياء', name: 'اختبار العناصر الانتقالية والتحليل الكيميائي الكلي', details: 'الدرجة: 85 / 100', date: '2026-05-28', status: 'جيد جداً' },
    { id: 'rec-6', type: 'اختبار أحياء', name: 'امتحان الوراثة والتطور والخلية النمطية الموسع', details: 'الدرجة: 94 / 100', date: '2026-05-12', status: 'ممتاز' },
    { id: 'rec-7', type: 'تقييم عام', name: 'تقييم العصور الوسطى والتضاريس الجغرافية للتاريخ', details: 'الدرجة: 89 / 100', date: '2026-04-20', status: 'جيد جداً' }
  ];
      ...
      try {
        if (role === 'admin') {
          ...
          if (targetStudentId) {
            ...
            const maxScore = submissions.reduce((acc, sub) => acc + (sub.totalQuestions || 1), 0);
            const averageQuizScore = submissions.length > 0 ? Math.round((totalScore / maxScore) * 100) : 0;
            ...
            if (submissions.some(sub => (sub.score / sub.totalQuestions) >= 0.95)) badges.push('العبقري المتفوق');
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="bg-gray-50/50 dark:bg-[#12121A]/50 p-6 rounded-2xl border border-gray-200 dark:border-[#2D2D3D] space-y-3">
            ...
            <div className="space-y-3 pt-2">
              ...
              <div>
                <div className="flex justify-between text-xs font-bold text-gray-500 dark:text-gray-400 mb-1">
                  <span>التفاعل مع منتدى الأسئلة والـ AI</span>
              ...
              <div className="flex-1 bg-white text-slate-900 border border-slate-200 rounded-2xl p-6 md:p-8 flex flex-col justify-between shadow-sm print:border-none print:shadow-none print:p-0">
                ...
                <div className="py-6 space-y-6">
                  <div className="bg-slate-50 p-4 rounded-xl border border-slate-150 print:bg-white print:border-slate-300">
                    ...
                    {getStudentReportRecords(displayUser?.id || '', printDateRange, printMonth, reportStartDate, reportEndDate).length > 0 ? (
                      <div className="overflow-hidden rounded-lg border border-slate-200 bg-white">
                        <table className="w-full text-right text-[10px]">
                          ...
                          <tbody className="divide-y divide-slate-100 font-bold text-slate-700">
                            {getStudentReportRecords(displayUser?.id || '', printDateRange, printMonth, reportStartDate, reportEndDate).map((rec) => (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-150 space-y-2 text-xs font-bold text-slate-700 print:bg-white print:border-slate-300">
                      <h4 className="text-[10px] font-black text-slate-400 border-b border-slate-200 pb-1 flex items-center gap-1.5">
                        ...
                        تقدير السلوك والمشاركة
                      </h4>
                      <div className="space-y-1 text-[10px]">
                        <div className="flex justify-between">
                          <span>نسبة استكمال الدروس والملفات المرفقة:</span>
                          <span className="text-slate-900 font-mono">92%</span>
                        </div>
                        <div className="flex justify-between">
                          <span>تسليم التطبيقات والاختبارات المنزلية:</span>
                          <span className="text-slate-900 font-mono">96%</span>
                        </div>
                        <div className="flex justify-between">
                          <span>معدل الأداء والانضباط المدرسي:</span>
                          <span className="text-slate-900 font-mono">ممتاز (A)</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-150 space-y-2 text-xs font-bold text-slate-700 print:bg-white print:border-slate-300">
                      ...
                      <div className="space-y-1 text-[10px]">
                        <div className="flex justify-between">
                          <span>المعدل التراكمي المقدر:</span>
                        ...
                        <div className="flex justify-between">
                          ...
                          <span className="text-slate-900">ممتاز جداً</span>
                        </div>
                        <div className="flex justify-between">
                          <span>مستوى الذكاء الأكاديمي والتحليل:</span>
                          <span className="text-slate-900">مرتفع ومثابر</span>
                        </div>
                      </div>
                    </div>
                  </div>
